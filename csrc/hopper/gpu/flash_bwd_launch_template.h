...
void run_flash_bwd(Flash_bwd_params &params, cudaStream_t stream) {
    ...
    CHECK_CUDA_KERNEL_LAUNCH_THROW();
    ...
    if constexpr (size(ClusterShape{}) > 1) {
        ...
        if (smem_size >= 48 * 1024) {
            CHECK_CUDA_THROW(cudaFuncSetAttribute(kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size));
        }
        ...
        if (smem_size >= 48 * 1024) {
            CHECK_CUDA_THROW(cudaFuncSetAttribute(cutlass::device_kernel<AttnKernel>, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size));
        }
    ...
    if (smem_size_postprocess >= 48 * 1024) {
        CHECK_CUDA_THROW(cudaFuncSetAttribute(cutlass::device_kernel<PostprocessKernel>, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size_postprocess));
    }
    ...
    if constexpr (GQA) {
        ...
        if (smem_size_postprocess >= 48 * 1024) {
            CHECK_CUDA_THROW(cudaFuncSetAttribute(cutlass::device_kernel<PostprocessKerneldKV>, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size_postprocess));
        }
